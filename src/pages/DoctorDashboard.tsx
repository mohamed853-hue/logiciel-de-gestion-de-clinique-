      ...
      {activeTab === 'appointments' && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: isArabic ? 'الإجمالي' : 'Total', value: appointments.length, color: 'bg-slate-100 text-slate-700' },
              { label: isArabic ? 'مجدول' : 'Planifiés', value: appointments.filter(a => a.status === 'planifie').length, color: 'bg-blue-100 text-blue-700' },
              { label: isArabic ? 'مؤكد' : 'Confirmés', value: appointments.filter(a => a.status === 'confirme').length, color: 'bg-emerald-100 text-emerald-700' },
              { label: isArabic ? 'اليوم' : 'Aujourd\'hui', value: todayAppts.length, color: 'bg-violet-100 text-violet-700' },
            ].map(s => (
              <div key={s.label} className={cn('p-3 rounded-2xl text-center shadow-xs', s.color)}>
            ...
            <CardHeader>
              <div className="flex items-center justify-between flex-wrap gap-3">
                <CardTitle className="text-base flex items-center gap-2">
                  ...
                  {isArabic ? 'جدول المواعيد الطبية والاستشارات' : 'Tous les Rendez-vous Médicaux'}
                </CardTitle>
                <Button size="sm" onClick={() => setShowAppointmentModal(true)} className="bg-purple-600 hover:bg-purple-700 font-bold text-xs">
                  <Calendar className="w-3.5 h-3.5 mr-1" />
                  {isArabic ? '+ موعد جديد' : '+ Nouveau RDV'}
                </Button>
              </div>
            </CardHeader>
            <CardContent>
              {loadingAppts ? (
                ...
                <EmptyState title={isArabic ? 'لا توجد مواعيد' : 'Aucun rendez-vous'} description={isArabic ? 'المواعيد المبرمجة من الاستقبال تظهر هنا تلقائياً' : 'Les rendez-vous planifiés par la réception apparaîtront ici.'} />
              ) : (
                  ...
                  {appointments.map(a => {
                    ...
                    return (
                      <div key={a.id} className={cn(
                        'p-4 rounded-2xl border flex items-center justify-between flex-wrap gap-3 text-xs transition-all',
                        isExpired ? 'bg-amber-50/60 border-amber-200' :
                        a.priority === 'urgent' ? 'bg-amber-50/40 border-amber-200' :
                        a.priority === 'emergency' ? 'bg-red-50/40 border-red-200' :
                        'bg-slate-50 border-slate-200'
                      )}>
                        <div>
                          <div className="flex items-center gap-2 flex-wrap">
                            <p className="font-extrabold text-slate-800 text-sm">{a.patient_name || (isArabic ? 'مريض' : 'Patient')}</p>
                            {isExpired && (
                              <span className="px-2 py-0.5 rounded-full text-[10px] font-black bg-amber-100 text-amber-800 border border-amber-300">
                                ⚠️ {isArabic ? 'موعد فائت / لم يحضر' : 'Passé / Non honoré'}
                              </span>
                            )}
                            {a.priority === 'urgent' && <span className="px-2 py-0.5 bg-amber-100 text-amber-700 rounded-full text-[10px] font-black">⚡ URGENT</span>}
                            {a.priority === 'emergency' && <span className="px-2 py-0.5 bg-rose-100 text-rose-700 rounded-full text-[10px] font-black animate-pulse">🚨 URGENCE</span>}
                          </div>
                          <p className="text-slate-500 mt-0.5">{isArabic ? 'الطبيب :' : 'Médecin :'} <strong className="text-slate-700">{a.doctor_name}</strong></p>
                          <p className="text-slate-400 mt-0.5 flex items-center gap-1 font-mono">
                            <Clock className="w-3 h-3 text-purple-600" /> {new Date(a.appointment_date).toLocaleString('fr-FR')}
                          </p>
                          {a.notes && <p className="text-slate-500 mt-0.5 italic">{a.notes}</p>}
                        </div>
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {a.status !== 'annule' && (
                            <Button size="sm" variant="ghost" onClick={() => handleUpdateApptStatus(a.id, 'annule')}
          ...
          <CardHeader>
            <div className="flex items-center justify-between flex-wrap gap-3">
              <CardTitle className="text-base flex items-center gap-2">
                ...
                {isArabic ? 'قائمة الفحوصات والتحاليل المخبرية' : 'Demandes et Résultats d\'Analyses'}
              </CardTitle>
              <Button size="sm" variant="outline" onClick={() => reloadLabs()} className="text-xs">
                <RefreshCw className="w-3.5 h-3.5 mr-1" /> {isArabic ? 'تحديث النتائج' : 'Actualiser'}
              </Button>
            </div>
          </CardHeader>
          <CardContent>
            {loadingLabs ? (
              ...
              <EmptyState title={isArabic ? 'لا توجد تحاليل' : 'Aucune analyse'} />
            ) : (
              <div className="space-y-3">
                {labTests.map(lab => (
                  <div key={lab.id} className="p-4 rounded-2xl border border-slate-200 bg-white flex justify-between items-start flex-wrap gap-3 text-xs">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <p className="font-extrabold text-slate-800 text-sm">{lab.test_name}</p>
                        {lab.urgence && <span className="px-2 py-0.5 bg-rose-100 text-rose-700 rounded-md font-bold text-[10px]">🚨 URGENT</span>}
                      </div>
                      <p className="text-slate-500">{isArabic ? 'طالب التحليل :' : 'Demandé par :'} <strong className="text-slate-700">{lab.requested_by}</strong> · {new Date(lab.created_at).toLocaleDateString('fr-FR')}</p>
                      
                      {lab.results_text && (
                        <div className="mt-2 p-2.5 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-900 font-mono text-xs">
                          <strong>{isArabic ? 'النتيجة المعتمدة :' : 'Résultat Validé :'}</strong> {lab.results_text}
                        </div>
                      )}
                    </div>
                    <StatusBadge status={lab.status} />
