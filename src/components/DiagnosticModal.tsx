...
export function DiagnosticModal({
  ...
  const [newDiseaseCategory, setNewDiseaseCategory] = useState('Général');
  ...
  const handleCreateNewDisease = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newDiseaseName.trim()) return;

    const created = await addNewPathologyToCatalog(newDiseaseName, newDiseaseCategory);
    setCatalog(prev => [created, ...prev]);
    setSelectedDisease(created.name);
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-5" dir={isArabic ? 'rtl' : 'ltr'}>
        ...
        {/* ─── 1. SÉLECTION DU TYPE DE MALADIE (CATALOGUE & RECHERCHE) ──────────── */}
        <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-3">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <label className="text-xs font-black text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
              <Stethoscope className="w-4 h-4 text-teal-600" />
              <span>{isArabic ? '1. اختر نوع المرض أو التشخيص' : '1. Sélectionnez la Pathologie / Maladie'}</span>
            </label>

            <button
              type="button"
              onClick={() => setShowAddNew(!showAddNew)}
              className="text-xs font-bold text-teal-700 hover:text-teal-800 bg-teal-50 px-2.5 py-1 rounded-lg border border-teal-200 cursor-pointer flex items-center gap-1"
            >
              <Plus className="w-3.5 h-3.5" />
              <span>{isArabic ? '+ إضافة مرض جديد' : '+ Nouvelle Maladie'}</span>
            </button>
          </div>

          {/* Formulaire rapide d'ajout si cliqué */}
          {showAddNew && (
            <div className="bg-white p-3 rounded-xl border-2 border-teal-300 shadow-sm space-y-2 animate-fade-in">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-teal-900">{isArabic ? 'تسجيل مرض جديد في الكتالوج' : 'Enregistrer une nouvelle pathologie'}</span>
                <button type="button" onClick={() => setShowAddNew(false)} className="text-slate-400 hover:text-slate-600">
                  <X className="w-4 h-4" />
                </button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <input
                  type="text"
                  placeholder="Ex: Paludisme Résistant, Otite..."
                  value={newDiseaseName}
                  onChange={e => setNewDiseaseName(e.target.value)}
                  className="px-3 py-2 border border-slate-200 rounded-lg text-xs outline-none focus:border-teal-500"
                />
                <input
                  type="text"
                  placeholder="Catégorie (ex: Infectieux, ORL...)"
                  value={newDiseaseCategory}
                  onChange={e => setNewDiseaseCategory(e.target.value)}
                  className="px-3 py-2 border border-slate-200 rounded-lg text-xs outline-none focus:border-teal-500"
                />
              </div>
              <Button type="button" onClick={handleCreateNewDisease} className="bg-teal-600 hover:bg-teal-700 text-white font-bold text-xs py-1.5 w-full">
                {isArabic ? 'إضافة إلى القائمة' : 'Ajouter au catalogue'}
              </Button>
            </div>
          )}
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder={isArabic ? 'ابحث عن مرض (ملاريا، زكام، تيفوئيد...)' : 'Rechercher une maladie (paludisme, rhume, typhoïde, HTA...)'}
              value={searchDisease}
              onChange={e => setSearchDisease(e.target.value)}
              className="w-full pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-xl text-xs outline-none focus:border-teal-500"
            />
          </div>

          {/* Liste sous forme de badges cliquables */}
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-40 overflow-y-auto pr-1">
            {filteredCatalog.map((disease) => {
              const isSelected = selectedDisease === disease.name;
              return (
                <button
                  key={disease.id}
                  type="button"
                  ...
                  className={cn(
                    'p-2 rounded-xl text-left transition-all border text-xs cursor-pointer flex flex-col justify-between',
                    isSelected
                      ? 'bg-teal-700 text-white border-teal-800 shadow-sm scale-[1.02]'
                      : 'bg-white text-slate-800 border-slate-200 hover:border-teal-300 hover:bg-teal-50/40'
                  )}
                >
                  <span className="font-extrabold truncate">{disease.name}</span>
                  <span className={cn('text-[10px] mt-0.5 opacity-80', isSelected ? 'text-teal-100' : 'text-slate-400')}>
                    {disease.category}
                  </span>
                </button>
              );
